...
import { reqDataList, reqFuncList, reqCommonlyList, reqGetUserName, reqGetUserInfor } from '@/api'
...
const actions = {
    ...
    //获取reqGetUserInfor
    async reqGetUserInfor({ commit }, ) {
        let result = await reqGetUserInfor()
        if (result.code == 200) { //如果请求成功，result.code就是200
            commit('GETUSERINFO', result.msg)
        }
    },
}
const mutations = {
    ...
    GETUSERINFO(state, userInfo) {
        state.userInfo = userInfo //将数据交给state
    }
}
const getters = {
